...
function vmtTracts() {
    var model = VMTMODEL[transportationModelTracts()],
        result =  Math.exp(model.Constant + model.LNVEH * Math.log(localeTracts[tractCounter].VEHICLES) + model.LNHHSIZE * Math.log(localeTracts[tractCounter].HHSIZE) + model.LNHHINC * Math.log(localeTracts[tractCounter].INCOME2013) + model.WORKCOUNT * localeTracts[tractCounter].WORKCNT + model.TIMETOWORK * localeTracts[tractCounter].TIMETOWORK + model.DRIVE * localeTracts[tractCounter].CARCOMMUTE + model.HHVEHCOUNT * localeTracts[tractCounter].VEHICLES + model.HBPOPDN * localeTracts[tractCounter].DENSITY + model.HHSIZE * localeTracts[tractCounter].HHSIZE + model.HHINC * localeTracts[tractCounter].INCOME2013 + model.WHITE * localeTracts[tractCounter].WHITE + model.HOMEOWN * localeTracts[tractCounter].OWN + model.HISP * localeTracts[tractCounter].LATINO);
...
// (float), returns floats [therms]
function naturalGasThermsTracts() {
    var NGCONSTANT = 1.074,
        NGGASCOEF = 2.911,
        NGROOMSCOEF = 0.331,
        NGYEARBUILTCOEF = 0.008575,
    ...
    result = Math.exp((NGCONSTANT + NGGASCOEF * localeTracts[tractCounter].GAS + NGROOMSCOEF * localeTracts[tractCounter].ROOMS + NGYEARBUILTCOEF * (REFYEAR - localeTracts[tractCounter].YEARBUILT) + NGASIANCOEF * localeTracts[tractCounter].ASIAN + NGHHSIZELNCOEF * Math.log(localeTracts[tractCounter].HHSIZE) + NGWHITECOEF * (localeTracts[tractCounter].WHITE / 2) + NGGRADCOEF * localeTracts[tractCounter].GRAD + NGSQFTCOEF * localeTracts[tractCounter].SQFT + NGOWNCOEF * localeTracts[tractCounter].OWN + NGSINGDETCOEF * localeTracts[tractCounter].SINGDET));
    
    return result;
}

// (float), returns floats [miles]
function airTravelDistanceTracts() {
    var AIRTRAVELCONSTANT = 33.088,
        AIRINCOMECOEF = 0.0633;
    
    return (AIRTRAVELCONSTANT + AIRINCOMECOEF * localeTracts[tractCounter].INCOME2007);
}

// (string), returns floats [miles / HH]
function hsrTravelTracts(year) {
    var perCapitaMiles = 0,
        mpo = localeTracts[tractCounter].MPO,
        pop = 0;
...
// (floats), returns floats [miles / HH]
function lightRailTravelTracts() {
    return (localeTracts[tractCounter]["Subway Commuters"] * PUBLICTRANSTABLE[COUNTY].CommuterRailMilesPerCommuter / localeTracts[tractCounter].WORKERS * localeTracts[tractCounter].WORKCNT);
}

function heavyRailTravelTracts() {
    return (localeTracts[tractCounter]["Railroad Commuters"] * PUBLICTRANSTABLE[COUNTY].AmtrakMilesPerCommuter / localeTracts[tractCounter].WORKERS * localeTracts[tractCounter].WORKCNT);
}

function busTravelTracts() {
    return (localeTracts[tractCounter]["Bus Commuters"] * PUBLICTRANSTABLE[COUNTY].BusMilesPerCommuter / localeTracts[tractCounter].WORKERS * localeTracts[tractCounter].WORKCNT);
}

// (float), returns floats  [gCO2e/HH]
function waterUseTracts() {
    var WATERHHSIZECOEF = 25550; // 70 gal/day/person * 365 days/yr
    
    return (WATERHHSIZECOEF * localeTracts[tractCounter].HHSIZE);
}
...
function vmntTracts() {
    ...
    return ((VMNTCONSTANT + VMNTINCOME07COEF * localeTracts[tractCounter].INCOME2007 + AUTOPARTHHSIZECOEF * localeTracts[tractCounter].HHSIZE) + (VEHSERVCONSTANT + VEHSERVINCOMECOEF * localeTracts[tractCounter].INCOME2007 + VEHSERVEHHSIZECOEF * localeTracts[tractCounter].HHSIZE));
}

// (float), returns floats [kwh]
function kwhUseTracts() {
    var KWHCONSTANT = 5.029,
        KWHROOMSCOEF = 0.09255,
        KWHLNCDDSQFTCOEF = 0.124,
        KWHLNHHSIZECOEF = 0.362,
    ...
    return (Math.exp(KWHCONSTANT + KWHROOMSCOEF * localeTracts[tractCounter].ROOMS + KWHLNCDDSQFTCOEF * Math.log(localeTracts[tractCounter].CDD * localeTracts[tractCounter].SQFT) + KWHLNHHSIZECOEF * Math.log(localeTracts[tractCounter].HHSIZE) + KWHLNINCOME13COEF * Math.log(localeTracts[tractCounter].INCOME2013) + KWHASIANCOEF * localeTracts[tractCounter].ASIAN + KWHGASCOEF * localeTracts[tractCounter].GAS + KWHOWNCOEF * localeTracts[tractCounter].OWN + KWHSQFTCOEF * localeTracts[tractCounter].SQFT + KWHGRADCOEF * localeTracts[tractCounter].GRAD + KWHSINGDETCOEF * localeTracts[tractCounter].SINGDET));
}

// (float), returns floats [gallons]
function oilUseTracts() {
    var FOCONSTANT = 228.95,
        FOROOMSCOEF = 75.846,
        FOREGIONCOEF = 128.895,
        FOOWNCOEF = -164.353,
    ...
    return ((FOCONSTANT + FOROOMSCOEF * localeTracts[tractCounter].ROOMS + FOREGIONCOEF * REGION + FOOWNCOEF * localeTracts[tractCounter].OWN + FOAGECOEF * localeTracts[tractCounter].AGE + FODIV1COEF * DIV1 + FOWHITECOEF * localeTracts[tractCounter].WHITE) * localeTracts[tractCounter].OTHERFUEL);
}

// (float), returns floats [tons]
function wasteGenerationTracts() {
    if (MSWTABLE[COUNTY].DISPOSALRATE) {
        return (MSWTABLE[COUNTY].DISPOSALRATE);
    } else {
        return (0.5);
...
// (float), returns floats
function housingConstructionTracts() {
    return (((0.0097 * Math.pow(localeTracts[tractCounter].SQFT, 2) - 10.012 * localeTracts[tractCounter].SQFT + 80256) / 70) * 1000); // gCO2/HH, amortized over 70 years
}

// (float), returns floats [calories]
function meatConsumptionTracts() {
    var ADULTMEATCAL = 487,
        CHILDMEATCAL = 365;
    return ((ADULTMEATCAL * localeTracts[tractCounter].Adults + CHILDMEATCAL * localeTracts[tractCounter].Children) * 365);
}

// (float), returns floats [calories]
function dairyConsumptionTracts() {
    var ADULTDAIRYCAL = 232,
        CHILDDAIRYCAL = 174;
    return ((ADULTDAIRYCAL * localeTracts[tractCounter].Adults + CHILDDAIRYCAL * localeTracts[tractCounter].Children) * 365);
}

// (float), returns floats [calories]
function otherFoodConsumptionTracts() {
    var ADULTOTHERCAL = 1170,
        CHILDOTHERCAL = 877;
    return ((ADULTOTHERCAL * localeTracts[tractCounter].Adults + CHILDOTHERCAL * localeTracts[tractCounter].Children) * 365);
}

// (float), returns floats [calories]
function vegeCaloriesTracts() {
    var ADULTVEGECAL = 304,
        CHILDVEGECAL = 228;
    return ((ADULTVEGECAL * localeTracts[tractCounter].Adults + CHILDVEGECAL * localeTracts[tractCounter].Children) * 365);
}

// (float), returns floats [calories]
function cerealCaloriesTracts() {
    var ADULTCEREALCAL = 584,
        CHILDCEREALCAL = 438;
    
    return ((ADULTCEREALCAL * localeTracts[tractCounter].Adults + CHILDCEREALCAL * localeTracts[tractCounter].Children) * 365);
}

// (float), returns floats [dollars]
function clothingUSDTracts() {
    var CLOTHCONSTANT = 75.63,
        CLOTHINCOME07COEF =  0.0149,
        CLOTHHHSIZECOEF =  323.60;
    
    return (CLOTHCONSTANT + CLOTHINCOME07COEF * localeTracts[tractCounter].INCOME2007 + CLOTHHHSIZECOEF * localeTracts[tractCounter].HHSIZE);
}

// (float), returns floats [dollars]
function furnishingsUSDTracts() {
    var FURCONSTANT =  278.96,
        FURINCOME07COEF =  0.0231,
        FURHHSIZECOEF =  27.70;
    
    return (FURCONSTANT + FURINCOME07COEF * localeTracts[tractCounter].INCOME2007 + FURHHSIZECOEF * localeTracts[tractCounter].HHSIZE);
}

// (float), returns floats [dollars]
function otherGoodsUSDTracts() {
    var OTHERGOODSCONSTANT =  2769.82,
        OTHERGOODSINCOME07COEF =  0.0291,
        OTHERGOODSHHSIZECOEF =  100.70;
    
    return (OTHERGOODSCONSTANT + OTHERGOODSINCOME07COEF * localeTracts[tractCounter].INCOME2007 + OTHERGOODSHHSIZECOEF * localeTracts[tractCounter].HHSIZE);
}

// (float), returns floats [dollars]
function servicesUSDTracts() {
    var SERVICESCONSTANT =  3939.74,
        SERVICESINCOME07COEF =  0.1428,
        SERVICESHHSIZECOEF =  102.47;
    
    return (SERVICESCONSTANT + SERVICESINCOME07COEF * localeTracts[tractCounter].INCOME2007 + SERVICESHHSIZECOEF * localeTracts[tractCounter].HHSIZE);
}

// (float), returns floats [dollars]
function medicalTracts() {
    var MEDICALCONSTANT = 775.25,
        MEDICALINCOME07COEF = 0.0029,
        MEDICALHHSIZECOEF = 0.13;
    
    return (MEDICALCONSTANT + MEDICALINCOME07COEF * localeTracts[tractCounter].INCOME2007 + MEDICALHHSIZECOEF * localeTracts[tractCounter].HHSIZE);
}

// (float), returns floats [dollars]
function entertainmentTracts() {
    var ENTERTAINCONSTANT =  228.57,
        ENTERTAININCOME07COEF =  0.0146,
        ENTERTAINHHSIZECOEF =  24.73;
    
    return (ENTERTAINCONSTANT + ENTERTAININCOME07COEF * localeTracts[tractCounter].INCOME2007 + ENTERTAINHHSIZECOEF * localeTracts[tractCounter].HHSIZE);
}

// (float), returns floats [dollars]
function readingTracts() {
    var READINGCONSTANT =  73.68,
        READINGINCOME07COEF =  0.0015,
        READINGHHSIZECOEF =  -17.53;
    
    return (READINGCONSTANT + READINGINCOME07COEF * localeTracts[tractCounter].INCOME2007 + READINGHHSIZECOEF * localeTracts[tractCounter].HHSIZE);
}

// (float), returns floats [dollars]
function careCleanTracts() {
    var CARECLEANCONSTANT =  229.21,
        CARECLEANINCOME07COEF =  0.0079,
        CARECLEANHHSZIECOEF =  85.57;
    
    return (CARECLEANCONSTANT + CARECLEANINCOME07COEF * localeTracts[tractCounter].INCOME2007 + CARECLEANHHSZIECOEF * localeTracts[tractCounter].HHSIZE);
}
...
function countyChange() {
    ...
    locale.COUNTY = COUNTY; // because locale.COUNTY was set to 1 in bgdata if it is a county
...
function emissionsCalc(year) {
    ...
    HHEmissions.Veggies = ghgFactors.FruitsVegetables * vegeCalories() * efficiencyMultiplier("AgriculturalEfficiency", year);
    HHEmissions.Cereal = ghgFactors.Cereal * cerealCalories() * efficiencyMultiplier("AgriculturalEfficiency", year);
    ...
    HHEmissions.HousingConstruction = housingConstruction();
...
function emissionsTractCalc(year) {
    ...
    popgrowth = POPTABLE[COUNTY][year] / POPTABLE[COUNTY]["2010"];
    ...
    for (tractCounter = 0; tractCounter < localeTracts.length; tractCounter++) {
        ...
        if (localeTracts[tractCounter].PDAAREA === 1) {
            popmultiplier = pdaGrowthMultiplier * (localeTracts[tractCounter].LANDAREA / totalPDAArea);
        } else {
            popmultiplier = suburbGrowthMultiplier * (localeTracts[tractCounter].LANDAREA / (allCounties[COUNTY].LANDAREA - totalPDAArea));
        }
        
        HHEmissions[CTY][localeTracts[tractCounter].CTY] = {};
    
        HHVMT = vmtTracts() * consumptionMultiplier("VMT", year);
        
        // Transportation emissions
        HHEmissions[CTY][localeTracts[tractCounter].CTY].AirTravel = (ghgFactors.Flightdirect + ghgFactors.Flightindirect) * airTravelDistanceTracts() * consumptionMultiplier("AirTravel", year) * efficiencyMultiplier("AirTravelEfficiency", year);
        HHEmissions[CTY][localeTracts[tractCounter].CTY].HSR = ghgFactors.HSR * hsrTravel(year);
        HHEmissions[CTY][localeTracts[tractCounter].CTY].HeavyRail = ghgFactors.HeavyRail * heavyRailTravelTracts();
        HHEmissions[CTY][localeTracts[tractCounter].CTY].LightRail = ghgFactors.LightRail * lightRailTravelTracts();
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Bus = ghgFactors.Bus * busTravelTracts();
        ...
        HHEmissions[CTY][localeTracts[tractCounter].CTY].VMT = (HHVMT / ghgFactors.MPG) * ghgFactors.Gasoline * (efficiencyMultiplier("ZeroCarbonFuels", year)) * (1 - ((policyMultipliers.EVs * adoptionCurves.EVs[year]) + (policyMultipliers["50MPG"] * adoptionCurves["50MPG"][year])));


        // TOTAL TRANSPORTATION
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Transportation = HHEmissions[CTY][localeTracts[tractCounter].CTY].AirTravel + HHEmissions[CTY][localeTracts[tractCounter].CTY].HSR + HHEmissions[CTY][localeTracts[tractCounter].CTY].HeavyRail + HHEmissions[CTY][localeTracts[tractCounter].CTY].LightRail + HHEmissions[CTY][localeTracts[tractCounter].CTY].Bus + HHEmissions[CTY][localeTracts[tractCounter].CTY].VMT;

        // ******************************************************* //

        // Energy emissions
        // kWh
        HHEmissions[CTY][localeTracts[tractCounter].CTY].kWh = ((ghgFactors.kWh + (ghgFactors.kWhindirect * efficiencyMultiplier("IndustrialEfficiency", year))) * efficiencyMultiplier("LowCarbonElectricity", year)) * ((kwhUseTracts() * consumptionMultiplier("EnergyConsumption", year) * efficiencyMultiplier("EnergyEfficiencyNew", year)) + (kwhUseTracts() * consumptionMultiplier("EnergyConsumption", year) * efficiencyMultiplier("EnergyEfficiencyExisting", year)));

        // this is insane
        // NatGas
        HHEmissions[CTY][localeTracts[tractCounter].CTY].NaturalGas = (ghgFactors.NGdirect + ghgFactors.NGindirect) * (naturalGasThermsTracts() * ((consumptionMultiplier("EnergyConsumption", year) * efficiencyMultiplier("EnergyEfficiencyNew", year) * (1 - ((policyMultipliers.HeatingElectrificationNew * adoptionCurves.HeatingElectrificationNew[year]) - ELECTRICHEATNEW2010) / (1 - ELECTRICHEATNEW2010))) + ((consumptionMultiplier("EnergyConsumption", year) * efficiencyMultiplier("EnergyEfficiencyExisting", year)) * (1 - ((policyMultipliers.HeatingElectrificationExisting * adoptionCurves.HeatingElectrificationExisting[year]) - ELECTRICHEATEXISTING2010) / (1 - ELECTRICHEATEXISTING2010)))));

        // Fuel Oil
        // For Chris: error in original excel cells regarding second + block missing first instance of 2050HH
        // Same insanity as NatGas
        HHEmissions[CTY][localeTracts[tractCounter].CTY].FuelOil = (ghgFactors.FuelOildirect + ghgFactors.FuelOilindirect) * (oilUseTracts() * ((consumptionMultiplier("EnergyConsumption", year) * efficiencyMultiplier("EnergyEfficiencyNew", year) * (1 - ((policyMultipliers.HeatingElectrificationNew * adoptionCurves.HeatingElectrificationNew[year]) - ELECTRICHEATNEW2010) / (1 - ELECTRICHEATNEW2010))) + ((consumptionMultiplier("EnergyConsumption", year) * efficiencyMultiplier("EnergyEfficiencyExisting", year)) * (1 - ((policyMultipliers.HeatingElectrificationExisting * adoptionCurves.HeatingElectrificationExisting[year]) - ELECTRICHEATEXISTING2010) / (1 - ELECTRICHEATEXISTING2010)))));

        // Water & waste
        // Model is inconsistent - has both Waste & Water EFficiency & Industrial Efficiency, uses Industrial for wateremissions and Waste&Water for waste. Does not include Waste and Water Efficiency policy calc for waste
        // For Chris: check in 
        // Not sure what the 1000000 is for ????
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Water = ghgFactors.Water * waterUseTracts() * efficiencyMultiplier("WasteandWaterEfficiency", year) * consumptionMultiplier("WaterConsumption", year);
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Waste = ghgFactors.Waste * wasteGenerationTracts() * efficiencyMultiplier("WasteandWaterEfficiency", year) * consumptionMultiplier("WasteConsumption", year);

        // TOTAL WASTE WATER ENERGY
        HHEmissions[CTY][localeTracts[tractCounter].CTY].WWE = HHEmissions[CTY][localeTracts[tractCounter].CTY].Water + HHEmissions[CTY][localeTracts[tractCounter].CTY].Waste + HHEmissions[CTY][localeTracts[tractCounter].CTY].FuelOil + HHEmissions[CTY][localeTracts[tractCounter].CTY].NaturalGas + HHEmissions[CTY][localeTracts[tractCounter].CTY].kWh;

        // ******************************************************* //

        // Food
        // For Chris: Not sure what 1000000 is for / from
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Meat = ghgFactors.Meat * meatConsumptionTracts() * efficiencyMultiplier("AgriculturalEfficiency", year) * consumptionMultiplier("FoodConsumption", year);
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Dairy = ghgFactors.Dairy * dairyConsumptionTracts() * efficiencyMultiplier("AgriculturalEfficiency", year) * consumptionMultiplier("FoodConsumption", year);
        HHEmissions[CTY][localeTracts[tractCounter].CTY].OtherFood = ghgFactors.Food * otherFoodConsumptionTracts() * efficiencyMultiplier("AgriculturalEfficiency", year) * consumptionMultiplier("FoodConsumption", year);
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Veggies = ghgFactors.FruitsVegetables * vegeCaloriesTracts() * efficiencyMultiplier("AgriculturalEfficiency", year);// * consumptionMultiplier("FoodConsumption", year);
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Cereal = ghgFactors.Cereal * cerealCaloriesTracts() * efficiencyMultiplier("AgriculturalEfficiency", year);// * consumptionMultiplier("FoodConsumption", year);

        HHEmissions[CTY][localeTracts[tractCounter].CTY].Food = HHEmissions[CTY][localeTracts[tractCounter].CTY].Meat + HHEmissions[CTY][localeTracts[tractCounter].CTY].Dairy + HHEmissions[CTY][localeTracts[tractCounter].CTY].OtherFood + HHEmissions[CTY][localeTracts[tractCounter].CTY].Veggies + HHEmissions[CTY][localeTracts[tractCounter].CTY].Cereal;
        ...
        // For Chris: Not sure what 1000000 is for / from
        // For Chris: Not sure what second (set of multipliers) / 2 is for / from
        HHEmissions[CTY][localeTracts[tractCounter].CTY].Clothing = ghgFactors.Clothing * (clothingUSDTracts() * consumptionMultiplier("GoodsConsumption", year) + (clothingUSDTracts() * policyMultipliers.GoodsConsumption * adoptionCurves.GoodsConsumption[year]) / 2) * efficiencyMultiplier("IndustrialEfficiency", year);

        HHEmissions[CTY][localeTracts[tractCounter].CTY].Furnishings = ghgFactors.Furnishings * (furnishingsUSDTracts() * consumptionMultiplier("GoodsConsumption", year) + (furnishingsUSDTracts() * policyMultipliers.GoodsConsumption * adoptionCurves.GoodsConsumption[year]) / 2) * efficiencyMultiplier("IndustrialEfficiency", year);

        HHEmissions[CTY][localeTracts[tractCounter].CTY].OtherGoods = ghgFactors.OtherGoodsSum * (otherGoodsUSDTracts() * consumptionMultiplier("GoodsConsumption", year) + (otherGoodsUSDTracts() * policyMultipliers.GoodsConsumption * adoptionCurves.GoodsConsumption[year]) / 2) * efficiencyMultiplier("IndustrialEfficiency", year);

        // There is a policy on the dashboard for adjusting home size, not a very feasible / realistic policy goal
        HHEmissions[CTY][localeTracts[tractCounter].CTY].HousingConstruction = housingConstructionTracts();// * consumptionMultiplier("GoodsConsumption", year);

        HHEmissions[CTY][localeTracts[tractCounter].CTY].vehicleMaintenance = (ghgFactors.VehicleMain * vmntTracts() + ghgFactors.VehicleManufacturing * HHVMT) * efficiencyMultiplier("IndustrialEfficiency", year);

        HHEmissions[CTY][localeTracts[tractCounter].CTY].AllGoods = HHEmissions[CTY][localeTracts[tractCounter].CTY].Clothing + HHEmissions[CTY][localeTracts[tractCounter].CTY].Furnishings + HHEmissions[CTY][localeTracts[tractCounter].CTY].OtherGoods + HHEmissions[CTY][localeTracts[tractCounter].CTY].vehicleMaintenance + HHEmissions[CTY][localeTracts[tractCounter].CTY].HousingConstruction;

        // ******************************************************* //

        HHEmissions[CTY][localeTracts[tractCounter].CTY].Services = ghgFactors.ServicesSum * servicesUSDTracts() * efficiencyMultiplier("CommercialEfficiency", year);

        
        
        for (emissionSector in HHEmissions[CTY][localeTracts[tractCounter].CTY]) {
            HHEmissions[CTY].sum += HHEmissions[CTY][localeTracts[tractCounter].CTY][emissionSector] * (localeTracts[tractCounter].HOUSEHOLDS * popmultiplier) / 1000000;
        }
        
        
        // this needs to be updated
        // sums HHEmissions[CTY][localeTracts[tractCounter].CTY] across all categories
        // clean this up for calculating emissions total
            /*
        for (ef in HHEmissions[CTY][localeTracts[tractCounter].CTY]) {
            sum += HHEmissions[CTY][localeTracts[tractCounter].CTY][ef];
        }
        */

        //return sum * (localeTracts[tractCounter].HOUSEHOLDS * popmultiplier) / 1000000; // convert gCO2e to tCO2e

    }
    
    return (HHEmissions[CTY].sum);
...
LineChart = function LineChart(chartselect) {
    ...
    EmissionTractLevel = function EmissionTractLevel(yearCount) {
        ...
        // Calculates the Y-values
        this.vertic = emissionsTractCalc(years[yearCount]);
    };
...
$.when(
    ...
    $.getJSON('https://coolclimatenetwork.github.io/data/tract/cities/' + CTY + '/allTracts-min.json', function (tracts_imported) {
        localeTracts = tracts_imported;
    })
    /* for live version
    ...
    */
    ...
    if (allCities && allCounties) {
        ...
        $(document).ready(function () {
            ...
            locale = allCities[$('#cityMenu :selected').text()];

            CTY = $('#cityMenu :selected').text();
            CITY = $('#cityMenu :selected').text();
            COUNTY = locale.COUNTY;
